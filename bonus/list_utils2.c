...
void	manip_list(t_list **one, t_list *other)
{
	...
	{
		ft_lstadd_front(one, ft_lstnew(other->pos, other->dist, other->type));
		return ;
	}
	tmp = (*one)->next;
	ft_lstadd_back(one, ft_lstnew(other->pos, other->dist, other->type));
	(*one)->next->next = tmp;
}
...
{	
	...
	while (lst)
	{
		ft_lstadd_back(&res, ft_lstnew(lst->pos, lst->dist, lst->type));
		lst = lst->next;
	}
	return (res);
}
